...
def session_required(f):
    ...
    def decorated(*args, **kwargs):
        if request.method == 'OPTIONS':
            return jsonify({"message": "Error logueado para auditoria. Servidor no respondio."}), 500
    finally:
        if conn:
            release_db_connection(conn)
...
def get_licitacion_detalle(current_user_id, lid):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            """, (lid,))
            lic = cur.fetchone()
            if not lic: return jsonify({"message": "Error logueado para auditoria. Servidor no respondio."}), 500
    finally:
        if conn: release_db_connection(conn)
...
def upload_licitacion_doc(current_user_id):
    ...
    try:
        ...
        if 'archivo' not in request.files: return jsonify({"message": "Error logueado para auditoria. Servidor no respondio."}), 500
    finally:
        if conn: release_db_connection(conn)
...
@app.route("/proyectos/<int:pid>/proximos_pasos", methods=["POST"])
@session_required
...
@app.route("/proyectos/proximos_pasos/<int:paso_id>", methods=["DELETE"])
@session_required
...
# Si estamos en el volumen de Railway, forzarlo allí
...
@app.route("/auditoria/estado", methods=["GET"])
@session_required
...
@app.route("/auditoria/pdf/<int:proyecto_id>", methods=["GET"])
@session_required
def auditoria_pdf_view(current_user_id, proyecto_id):
    ...
    # Obtener nombre del proyecto para el nombre de descarga
    conn = None
...
def volume_gui(current_user_id):
    ...
    <body class="p-6">
        <div class="card">
            <h2>Gestión de Datos (Servidor Actual)</h2>
            <p style="font-size: 14px; color: #666;">Carpeta de origen: <b>{docs_folder}</b></p>
            
            <a href="/api/volume/export" target="_blank" style="text-decoration: none;">
                <button>📥 EXPORTAR TODO A ZIP (Backup)</button>
            </a>
            ...
            <div class="upload-form">
                ...
                <form action="/api/volume/import" method="post" enctype="multipart/form-data">
                    ...
                    <button type="submit" onclick="return confirm('¿Está seguro de importar? Esto podría sobreescribir archivos.')">📤 Cargar e Importar</button>
                </form>
            </div>
            
            <div class="status">
                Use esta herramienta para mover sus documentos entre servidores de forma limpia.
            </div>
        </div>
    </body>
    </html>
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            for folder_name, full_path in items_to_backup.items():
                if os.path.exists(full_path) and os.path.isdir(full_path):
                    print(f">>> EXPORTANDO CARPETA: {folder_name}")
                    for root, dirs, files in os.walk(full_path):
                        for file in files:
                            file_path = os.path.join(root, file)
                            # Meter en el ZIP con la estructura "nombre_carpeta/..."
...
def volume_import(current_user_id):
    ...
    try:
        # El destino base es donde vive app21.py (o /data en Railway)
        BASE_TARGET = os.path.dirname(os.path.abspath(__file__))
        if os.path.exists("/data"):
            BASE_TARGET = "/data"
        
        print(f">>> IMPORTANDO DATOS EN: {BASE_TARGET}")
        with zipfile.ZipFile(file) as zf:
            # Extraer preservando la estructura del ZIP (docs/, auditoria_reportes/, etc)
